...
export function TrainingMilestoneActions({ milestoneKey, isPractice, isComplete, locked, actionLabel }: {
  milestoneKey: string;
  isPractice: boolean;
  isComplete: boolean;
  locked: boolean;
  ...
  return <div className="flex w-full flex-col items-stretch gap-2 sm:w-auto sm:items-end">
    <div className="flex flex-col gap-2 sm:flex-row">
      ...
      {!isComplete && <Button type="button" onClick={() => update("complete")} disabled={isPending} className="w-full shrink-0 sm:w-auto">{isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Check className="size-4" />}{isPractice ? "Mark Complete" : actionLabel}</Button>}
